...
class WeatherPage extends ControllerBase {
  ...
  public function __construct(
    ForecastClientInterface $forecast_client,
  ) {
  ...
  public function build(string $style): array {
    ...
    $url = 'https://drupallearning.ddev.site:8443/modules/custom/anytown/data/weather_forecast.json';
    ...
    $closures = $settings->get('weather_closures');
    $build = [
      ...
      '#weather_closures' => [
        ...
        '#items' => $closures ? explode(PHP_EOL, $closures) : [],
        '#cache' => [
          ...
          // Remember, this page can be accessed via multiple URLs, like
          // /weather and /weather/extended. And varies depending on the URL,
          // so we also need to add a cache context for the URL so that the
          // content is cached per-url.
          'contexts' => ['url'],
        ],
      ],
    ];
